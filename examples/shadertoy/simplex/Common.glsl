...
bool RaySimplexSDF(inout Ray ray, inout vec2 sdfNear, out HitCtx hit, in Transform transform, in float explode, in int interval, in float phase)
{
    ...
    {
        int k = j*3;
        vec3 d = mix(V[I[k + interval%3]], V[I[k + (interval+1)%3]], beta) * explode;
        if(interval == 0) { D[j] = d * beta; } 
        else if(interval < kClosedInterval) { D[j] = d; }
        else if(interval == kClosedInterval) { D[j] = d * (1.0 - beta); }
        else { D[j] = kZero; }
        
        N[j] = normalize(cross(V[I[k+1]] - V[I[k]], V[I[k+2]] - V[I[k]]));
    }
    ...
    for(stepIdx = 0; stepIdx < kSDFMaxSteps; stepIdx++)
    {
        ...
        for(int j = 0; j < 4; j++)
        {              
            //#define functor SDFSimplexFace
            #define functor SDFTriangle
            vec4 FFace = functor(p, vec3[3](V[I[j*4]] + D[j], 
                                                  V[I[j*4+1]] + D[j], 
                                                  V[I[j*4+2]] + D[j]),
                                                  N[j]);
